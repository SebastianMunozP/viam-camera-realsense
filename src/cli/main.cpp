...
int main() {
  ...
  for (size_t i = 0; i < device_list.size(); i++) {
    try {
      ...
      std::cerr << "[main] Failed to process device with error: " << e.what()
                << std::endl;
    }
  }

  ctx.set_devices_changed_callback([&](const rs2::event_information &info) {
    auto list = ctx.query_devices();
    std::cout << "Number of connected devices changed to: " << list.size()
              << "\n";
