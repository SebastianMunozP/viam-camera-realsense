...
std::vector<std::shared_ptr<vsdk::ModelRegistration>> create_all_model_registrations(
    std::shared_ptr<realsense::RealsenseContext<boost::synchronized_value<rs2::context>>>
        realsense_ctx,
  ...
  registrations.push_back(std::make_shared<vsdk::ModelRegistration>(
      ...
      [realsense_ctx, assigned_serials](vsdk::Dependencies deps, vsdk::ResourceConfig config) {
        return std::make_unique<realsense::Realsense<boost::synchronized_value<rs2::context>>>(
            std::move(deps), std::move(config), realsense_ctx, assigned_serials);
      },
      realsense::Realsense<
          realsense::RealsenseContext<boost::synchronized_value<rs2::context>>>::validate));

  registrations.push_back(std::make_shared<vsdk::ModelRegistration>(
      vsdk::API::get<vsdk::Discovery>(),
      realsense::discovery::RealsenseDiscovery<boost::synchronized_value<rs2::context>>::model,
...
int serve(int argc, char** argv) try {
  ...
  auto uid = getuid();
  if (uid != 0) {
    std::cerr << "[serve] Realsense module is not running as root (user ID = " << uid
  ...
  auto rs_ctx =
      std::make_shared<realsense::RealsenseContext<boost::synchronized_value<rs2::context>>>(ctx);
