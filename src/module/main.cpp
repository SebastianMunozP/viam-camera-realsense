...
std::vector<std::shared_ptr<vsdk::ModelRegistration>> create_all_model_registrations(std::shared_ptr<rs2::context> ctx) {
    ...
    registrations.push_back(std::make_shared<vsdk::ModelRegistration>(
        vsdk::API::get<vsdk::Camera>(), realsense::Realsense::model, [](vsdk::Dependencies deps, vsdk::ResourceConfig config) {
            return std::make_unique<realsense::Realsense>(std::move(deps), std::move(config));
        }));
