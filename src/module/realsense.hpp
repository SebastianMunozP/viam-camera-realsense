...
#include "utils.hpp"
namespace realsense {
static const std::unordered_set<std::string> SUPPORTED_CAMERA_MODELS = {"D435", "D435I"};
...
static constexpr std::uint64_t TIMESTAMP_WARNING_LOG_INTERVAL_MS = 60000;  // 1
                                                                           // minute
...
class RealsenseContext {
 public:
  RealsenseContext(std::shared_ptr<SynchronizedContextT> ctx) : rs_context_(ctx) {
    setupCallback();
  }
 ...
 private:
  ...
  boost::synchronized_value<std::unordered_set<Realsense<SynchronizedContextT>*>> instances_{
    ...
    for (Realsense<SynchronizedContextT>* instance : *instances_guard) {
      if (instance != nullptr) {
        try {
          ...
          VIAM_SDK_LOG(error) << "[RealsenseContext] Error notifying instance: " << e.what();
...
struct DeviceFunctions {
  std::function<bool(std::shared_ptr<boost::synchronized_value<device::ViamRSDevice<>>>&,
                     viam::sdk::LogSource&)>
      stopDevice;
  std::function<bool(std::shared_ptr<boost::synchronized_value<device::ViamRSDevice<>>>&,
                     viam::sdk::LogSource&)>
  ...
  std::function<std::shared_ptr<boost::synchronized_value<device::ViamRSDevice<>>>(
      std::string const&, std::shared_ptr<rs2::device>, std::unordered_set<std::string> const&,
  ...
  std::function<void(std::string const&,
                     ...
                     realsense::RsResourceConfig const&, viam::sdk::LogSource&)>
      startDevice;
  std::function<void(std::shared_ptr<boost::synchronized_value<device::ViamRSDevice<>>>&,
                     realsense::RsResourceConfig const&, viam::sdk::LogSource&)>
      reconfigureDevice;
};
...
class Realsense final : public viam::sdk::Camera, public viam::sdk::Reconfigurable {
 public:
  Realsense(
      ...
      : Realsense(deps, cfg, ctx, createDefaultDeviceFunctions(), assigned_serials) {}
  Realsense(
      viam::sdk::Dependencies deps, viam::sdk::ResourceConfig cfg,
      std::shared_ptr<RealsenseContext<SynchronizedContextT>> ctx, DeviceFunctions device_funcs,
      std::shared_ptr<boost::synchronized_value<std::unordered_set<std::string>>> assigned_serials)
      : Camera(cfg.name()),
        config_(configure(deps, cfg)),
        realsense_ctx_(ctx),
        device_funcs_(device_funcs),
        assigned_serials_(assigned_serials) {
    std::string requested_serial_number = config_->serial_number;
    VIAM_RESOURCE_LOG(info) << "[constructor] start for resource " << config_->resource_name
    ...
    if (not assign_and_initialize_device(device_list)) {
      if (not requested_serial_number.empty()) {
        ...
        throw std::runtime_error("failed to start device " + requested_serial_number);
      } else {
    ...
    VIAM_RESOURCE_LOG(info) << "Realsense constructor end " << requested_serial_number;
  }
  ~Realsense() {
    if (device_) {
  ...
  void reconfigure(const viam::sdk::Dependencies& deps,
    ...
    if (not physical_camera_assigned_) {
      VIAM_RESOURCE_LOG(error) << "[reconfigure] cannot reconfigure a device that "
                                  "does not have a physical device assigned";
      throw std::runtime_error(
          "cannot reconfigure a device that does not have "
    ...
    VIAM_RESOURCE_LOG(error) << "[reconfigure] stopping device " << prev_serial_number;
    if (not device_funcs_.stopDevice(device_, this->logger_)) {
      VIAM_RESOURCE_LOG(error) << "[reconfigure] failed to stop device " << prev_serial_number;
    ...
    */
    std::string requested_serial_number = config_->serial_number;
    if (not requested_serial_number.empty() and prev_serial_number != requested_serial_number) {
      if (device_) {
        {  // Begin scope for device_guard lock
          auto device_guard = device_->synchronize();
          {  // Begin scope for serials_guard lock
      ...
      VIAM_RESOURCE_LOG(error) << "[reconfigure] destroying device " << prev_serial_number;
      if (not device_funcs_.destroyDevice(device_, this->logger_)) {
        VIAM_RESOURCE_LOG(error) << "[reconfigure] failed to destroy device " << prev_serial_number;
      ...
      if (not assign_and_initialize_device(device_list)) {
        ...
        throw std::runtime_error("failed to start device " + config_->serial_number);
      }
      physical_camera_assigned_ = true;
    } else {
      realsense::RsResourceConfig config_copy = config_.get();
      VIAM_RESOURCE_LOG(info) << "[reconfigure] same serial number, reusing device "
                              << prev_serial_number;
      device_funcs_.reconfigureDevice(device_, config_copy, this->logger_);
      device_funcs_.startDevice(config_copy.serial_number, device_, latest_frameset_,
                                MAX_FRAME_AGE_MS, config_copy, this->logger_);
    }
    ...
    try {
      ...
      for (auto const& sensor : sensors) {
        if (sensor == sensors::SensorType::color) {
          ...
          std::uint64_t timestamp = static_cast<std::uint64_t>(std::llround(color.get_timestamp()));
          ...
          std::uint64_t timestamp = static_cast<std::uint64_t>(std::llround(depth.get_timestamp()));
      ...
      if (utils::contains(sensors::SensorType::color, sensors) and
          utils::contains(sensors::SensorType::depth, sensors) and should_process_color and
          should_process_depth) {
        auto const color = fs.get_color_frame();
        auto const depth = fs.get_depth_frame();
        auto const timeDiffMs = static_cast<std::uint64_t>(
            std::llround(std::abs(color.get_timestamp() - depth.get_timestamp())));
        auto const colorTS = static_cast<std::uint64_t>(std::llround(color.get_timestamp()));
        auto const depthTS = static_cast<std::uint64_t>(std::llround(depth.get_timestamp()));
        ...
        if (timeDiffMs > MAX_FRAME_SET_TIME_DIFF_MS) {
          ...
          if (should_warn) {
            VIAM_RESOURCE_LOG(warn) << "color and depth timestamps differ by " << timeDiffMs
            ...
            VIAM_RESOURCE_LOG(debug) << "color and depth timestamps differ by " << timeDiffMs
  ...
  viam::sdk::Camera::point_cloud get_point_cloud(std::string mime_type,
    ...
    try {
      ...
      if (color_frame.get_data() == nullptr or color_frame.get_data_size() == 0) {
        throw std::runtime_error("[get_image] color data is null");
      }

      if (depth_frame.get_data() == nullptr or depth_frame.get_data_size() == 0) {
      ...
      {  // Begin scope for my_dev lock
        ...
        data = encoding::encodeRGBPointsToPCD(my_dev->point_cloud_filter->process(fs), logger_);
      }  // End scope for my_dev lock

      if (data.size() > MAX_GRPC_MESSAGE_SIZE) {
        VIAM_RESOURCE_LOG(error) << "[get_point_cloud] data size exceeds gRPC message size limit";
        throw std::runtime_error("point cloud size " + std::to_string(data.size()) +
  ...
  viam::sdk::Camera::properties get_properties() override {
    try {
      ...
      auto fillResp = [this](viam::sdk::Camera::properties& p, rs2_intrinsics const& props) {
        p.supports_pcd = true;
        ...
        */
        ...
        VIAM_RESOURCE_LOG(debug) << "[get_properties] properties: ["
  ...
  static std::vector<std::string> validate(viam::sdk::ResourceConfig cfg) {
    ...
    if (sensors_proto.size() == 0 or sensors_proto.size() > 2) {
      VIAM_SDK_LOG(error) << "[validate] sensors field must contain 1 or 2 elements";
    ...
    if (sensors_proto.size() == 1) {
      ...
      if (sensors_param != "color" and sensors_param != "depth") {
        VIAM_SDK_LOG(error) << "[validate] sensors must be either 'color' or 'depth'";
        throw std::invalid_argument("sensors must be either 'color' or 'depth'");
      }
    }
    if (sensors_proto.size() == 2) {
      VIAM_SDK_LOG(info) << "[validate] Validating that sensors is a string";
      if (not sensors_proto[0].is_a<std::string>() or not sensors_proto[1].is_a<std::string>()) {
        VIAM_SDK_LOG(error) << "[validate] sensors is not a string";
        throw std::invalid_argument("sensors must be a string");
      }
      std::string sensors_param_1 = sensors_proto[0].get_unchecked<std::string>();
      std::string sensors_param_2 = sensors_proto[1].get_unchecked<std::string>();
 ...
 private:
  ...
  std::shared_ptr<boost::synchronized_value<std::unordered_set<std::string>>> assigned_serials_;
  ...
  void deviceChangedCallback(rs2::event_information& info) {
    std::cout << "[deviceChangedCallback] Device connection status changed" << std::endl;
    try {
      ...
      {  // Begin scope for current_device lock
        ...
        if (device_ and info.was_removed(*current_device->device)) {
          std::cerr << "[deviceChangedCallback] Device removed: " << current_device->serial_number
                    << std::endl;
          {  // Begin scope for serials_guard lock
            auto serials_guard = assigned_serials_->synchronize();
      ...
      std::cerr << "[deviceChangedCallback] Exception occurred: " << e.what() << std::endl;
    }
  }
  ...
  bool assign_and_initialize_device(DeviceListT const& device_list) {
    if (physical_camera_assigned_) {
      std::cerr << "[assign_and_initialize_device] Camera is already assigned" << std::endl;
      return true;
    }
    std::string requested_serial_number = config_->serial_number;
    ...
    // This will the initial set of connected devices (i.e. the devices that
    // were connected before the callback was set)
    VIAM_RESOURCE_LOG(info) << "[assign_and_initialize_device] Number of connected devices: "
                            << device_list.size() << "\n";
    ...
    for (int i = 0; i < devCount; i++) {
      try {
        ...
        std::string connected_device_serial_number = dev.get_info(RS2_CAMERA_INFO_SERIAL_NUMBER);

        VIAM_RESOURCE_LOG(info) << "[assign_and_initialize_device] trying connecting to device: "
        ...
        {  // Begin scope for serials_guard lock
          ...
          if ((requested_serial_number.empty() &&
            ...
            VIAM_RESOURCE_LOG(info) << "[assign_and_initialize_device] grabbing device: "
                                    << connected_device_serial_number;

            serials_guard->insert(connected_device_serial_number);
          } else {
            VIAM_RESOURCE_LOG(info) << "[assign_and_initialize_device] Could not grab device: "
                                    << connected_device_serial_number;
            continue;
          }
        }  // End scope for serials_guard lock
        VIAM_RESOURCE_LOG(info) << "[assign_and_initialize_device] calling createDevice for: "
                                << connected_device_serial_number;
        realsense::RsResourceConfig config_copy = config_.get();
        device_ = device_funcs_.createDevice(connected_device_serial_number, dev_ptr,
                                             SUPPORTED_CAMERA_MODELS, config_copy, this->logger_);
        BOOST_ASSERT(device_ != nullptr);

        VIAM_RESOURCE_LOG(info) << "[assign_and_initialize_device] calling startDevice for: "
                                << connected_device_serial_number;
        device_funcs_.startDevice(connected_device_serial_number, device_, latest_frameset_,
                                  MAX_FRAME_AGE_MS, config_copy, this->logger_);
        physical_camera_assigned_ = true;
        return true;
      } catch (const std::exception& e) {
  ...
  RsResourceConfig configure(viam::sdk::Dependencies dependencies,
    ...
    for (const auto& sensor : sensors_list) {
      ...
      if (sensor_type == sensors::SensorType::unknown) {
        throw std::runtime_error("Invalid sensor type: " + sensor.get_unchecked<std::string>());
      }
    ...
    auto native_config =
        realsense::RsResourceConfig(serial, configuration.name(), sensors, width, height);
    ...
    return DeviceFunctions{
        .stopDevice =
            [](std::shared_ptr<boost::synchronized_value<device::ViamRSDevice<>>>& device,
               viam::sdk::LogSource& logger) { return device::stopDevice(device, logger); },
        .destroyDevice =
            [](std::shared_ptr<boost::synchronized_value<device::ViamRSDevice<>>>& device,
               viam::sdk::LogSource& logger) { return device::destroyDevice(device, logger); },
        .printDeviceInfo =
        ...
        .createDevice =
            [](std::string const& serial, std::shared_ptr<rs2::device> dev_ptr,
               ...
               realsense::RsResourceConfig const& config, viam::sdk::LogSource& logger) {
              return device::createDevice<realsense::RsResourceConfig, device::ViamRSDevice<>,
        ...
        .startDevice =
            [](const std::string& serial,
               ...
               std::uint64_t maxFrameSetFrameMs, realsense::RsResourceConfig const& viamConfig,
               viam::sdk::LogSource& logger) {
              return device::startDevice(serial, device, latest_frameset, maxFrameSetFrameMs,
                                         viamConfig, logger);
            },
        .reconfigureDevice =
            [](std::shared_ptr<boost::synchronized_value<device::ViamRSDevice<>>> device,
               realsense::RsResourceConfig const& viamConfig, viam::sdk::LogSource& logger) {
              device::reconfigureDevice<realsense::RsResourceConfig, device::ViamRSDevice<>,
                                        rs2::device, rs2::config, rs2::color_sensor,
                                        rs2::depth_sensor, rs2::video_stream_profile>(
