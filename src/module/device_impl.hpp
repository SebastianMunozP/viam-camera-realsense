  ...
  if (sensor.supports(RS2_OPTION_GLOBAL_TIME_ENABLED)) {
    try {
      ...
      VIAM_DEVICE_LOG(logger, error)
          << "[enableGlobalTimestamp] Failed to enable Global Timestamp: " << e.what();
    }
  }
}
...
void disableAutoExposurePriority(SensorT& sensor, viam::sdk::LogSource& logger) {
  try {
    ...
    VIAM_DEVICE_LOG(logger, error)
        << "[disableAutoExposurePriority] Failed to get sensor type: " << e.what();
  }
}
...
void printDeviceInfo(DeviceT const& dev, viam::sdk::LogSource& logger) {
  try {
    ...
    if (dev.supports(RS2_CAMERA_INFO_SERIAL_NUMBER)) {
      info << "  Serial Number:                  " << dev.get_info(RS2_CAMERA_INFO_SERIAL_NUMBER)
           << std::endl;
    }
    if (dev.supports(RS2_CAMERA_INFO_PRODUCT_LINE)) {
      info << "  Product Line:                   " << dev.get_info(RS2_CAMERA_INFO_PRODUCT_LINE)
           << std::endl;
    }
    ...
    if (dev.supports(RS2_CAMERA_INFO_FIRMWARE_VERSION)) {
      info << "  Firmware Version:               " << dev.get_info(RS2_CAMERA_INFO_FIRMWARE_VERSION)
           << std::endl;
    }
    if (dev.supports(RS2_CAMERA_INFO_RECOMMENDED_FIRMWARE_VERSION)) {
      info << "  Recommended Firmware Version:   "
           << dev.get_info(RS2_CAMERA_INFO_RECOMMENDED_FIRMWARE_VERSION) << std::endl;
    }
    if (dev.supports(RS2_CAMERA_INFO_FIRMWARE_UPDATE_ID)) {
      info << "  Firmware Update ID:   " << dev.get_info(RS2_CAMERA_INFO_FIRMWARE_UPDATE_ID)
           << std::endl;
    }
    if (dev.supports(RS2_CAMERA_INFO_PHYSICAL_PORT)) {
      info << "  Physical Port:   " << dev.get_info(RS2_CAMERA_INFO_PHYSICAL_PORT) << std::endl;
    }
    if (dev.supports(RS2_CAMERA_INFO_DEBUG_OP_CODE)) {
      info << "  Debug OP Code:   " << dev.get_info(RS2_CAMERA_INFO_DEBUG_OP_CODE) << std::endl;
    }
    if (dev.supports(RS2_CAMERA_INFO_ADVANCED_MODE)) {
      info << "  Advanced Mode:   " << dev.get_info(RS2_CAMERA_INFO_ADVANCED_MODE) << std::endl;
    }
    if (dev.supports(RS2_CAMERA_INFO_CAMERA_LOCKED)) {
      info << "  Camera Locked:   " << dev.get_info(RS2_CAMERA_INFO_CAMERA_LOCKED) << std::endl;
    }
    ...
    if (dev.supports(RS2_CAMERA_INFO_DFU_DEVICE_PATH)) {
      info << "  DFU Device Path:            " << dev.get_info(RS2_CAMERA_INFO_DFU_DEVICE_PATH)
           << std::endl;
    }
    if (dev.supports(RS2_CAMERA_INFO_IP_ADDRESS)) {
      info << "  IP Address:            " << dev.get_info(RS2_CAMERA_INFO_IP_ADDRESS) << std::endl;
    }
...
void frameCallback(FrameT const& frame, std::uint64_t const maxFrameAgeMs,
  ...
  if (not initial_frameset or initial_frameset.size() != expected_frame_count) {
    std::cerr << "[frame_callback] got count other than " << expected_frame_count << ": "
              << initial_frameset.size() << std::endl;
    return;
  }
  ...
  if (not color_frame and utils::contains(sensors::SensorType::color, viamConfig.sensors)) {
    std::cerr << "[frame_callback] no color frame" << std::endl;
    return;
  }
  if (color_frame and not utils::contains(sensors::SensorType::color, viamConfig.sensors)) {
    std::cerr << "[frame_callback] received color frame when not expected" << std::endl;
    return;
  }
  ...
  if (color_frame) {
    ...
    if (colorAge > maxFrameAgeMs) {
      std::cerr << "[frame_callback] received color frame is too stale, age: " << colorAge << "ms"
                << std::endl;
    }
  }

  auto depth_frame = frameset.get_depth_frame();
  if (not depth_frame and utils::contains(sensors::SensorType::depth, viamConfig.sensors)) {
    std::cerr << "[frame_callback] no depth frame" << std::endl;
    return;
  }
  if (depth_frame and not utils::contains(sensors::SensorType::depth, viamConfig.sensors)) {
    std::cerr << "[frame_callback] received depth frame when not expected" << std::endl;
    return;
  }
  if (depth_frame) {
    double depthAge = nowMs - depth_frame.get_timestamp();
    if (depthAge > maxFrameAgeMs) {
      std::cerr << "[frame_callback] received depth frame is too stale, age: " << depthAge << "ms"
                << std::endl;
    }
  }
...
bool checkIfMatchingColorDepthProfiles(const VideoStreamProfileT& color,
  ...
  if (std::tuple(color.width(), color.height(), color.fps()) ==
    ...
    VIAM_DEVICE_LOG(logger, info) << "using width: " << color.width()
...
template <typename SensorT>
class SensorTypeTraits;

template <>
class SensorTypeTraits<rs2::color_sensor> {
 public:
...
std::shared_ptr<ConfigT> createSingleSensorConfig(std::shared_ptr<DeviceT> dev,
  ...
  VIAM_DEVICE_LOG(logger, info) << "[createSingleSensorConfig] Got sensor, getting stream profiles";
  // Get stream profiles
  auto profiles = sensor.get_stream_profiles();

  VIAM_DEVICE_LOG(logger, info) << "[createSingleSensorConfig] Got " << profiles.size()
  ...
  for (auto& cp : profiles) {
    ...
    if ((not viamConfig.width) or
      ...
      VIAM_DEVICE_LOG(logger, info) << "[createSingleSensorConfig] Found matching "
                                       "stream profile, enabling";
      cfg->enable_stream(SensorTypeTraits<SensorT>::stream_type, csp.stream_index(), csp.width(),
                         csp.height(), csp.format(), csp.fps());
      VIAM_DEVICE_LOG(logger, info) << "[createSingleSensorConfig] enabled stream";
      return cfg;
    }
  }
  return nullptr;
...
template <typename DeviceT, typename ConfigT, typename ColorSensorT, typename DepthSensorT,
  ...
  for (auto& cp : color_profiles) {
    ...
    for (auto& dp : depth_profiles) {
      ...
      if (checkIfMatchingColorDepthProfiles(csp, dsp, logger) and
          ...
          ((not viamConfig.height) or (viamConfig.height == csp.height()))) {
        VIAM_DEVICE_LOG(logger, info) << "[createSwD2CAlignConfig] Found matching color "
                                         "and depth stream profiles";
        cfg->enable_stream(SensorTypeTraits<ColorSensorT>::stream_type, csp.stream_index(),
                           csp.width(), csp.height(), csp.format(), csp.fps());
        cfg->enable_stream(SensorTypeTraits<DepthSensorT>::stream_type, dsp.stream_index(),
                           dsp.width(), dsp.height(), dsp.format(), dsp.fps());
        VIAM_DEVICE_LOG(logger, info) << "[createSwD2CAlignConfig] enabled color and depth streams";
        return cfg;
      }
    }
  }
...
std::shared_ptr<ConfigT> createConfig(std::shared_ptr<DeviceT> device,
  ...
  if (utils::contains(sensors::SensorType::color, viamConfig.sensors) and
    ...
    VIAM_DEVICE_LOG(logger, info) << "[createConfig] Creating config with color and "
                                     "depth sensors";
    config = createSwD2CAlignConfig<DeviceT, ConfigT, ColorSensorT, DepthSensorT,
                                    VideoStreamProfileT, ViamConfigT>(device, viamConfig, logger);

  } else if (viamConfig.sensors.size() == 1 &&
             utils::contains(sensors::SensorType::color, viamConfig.sensors)) {
    VIAM_DEVICE_LOG(logger, info) << "[createConfig] Creating config with color sensor";
    config =
        createSingleSensorConfig<DeviceT, ConfigT, ColorSensorT, VideoStreamProfileT, ViamConfigT>(
            device, viamConfig, logger);
  } else if (viamConfig.sensors.size() == 1 &&
             utils::contains(sensors::SensorType::depth, viamConfig.sensors)) {
    VIAM_DEVICE_LOG(logger, info) << "[createConfig] Creating config with depth sensor";
    config =
        createSingleSensorConfig<DeviceT, ConfigT, DepthSensorT, VideoStreamProfileT, ViamConfigT>(
            device, viamConfig, logger);
  } else {
    VIAM_DEVICE_LOG(logger, error) << "[createConfig] Unsupported sensor configuration requested, "
                                      "only "
                                      "'color', 'depth' or both are supported";
    throw std::runtime_error(
        "Unsupported sensor configuration requested, "
        "only 'color', 'depth' or both are supported");
  }
  // We are not currently supporting only depth sensor
  if (config == nullptr) {
    VIAM_DEVICE_LOG(logger, error) << "[createConfig] Current device configuration not "
                                      "supported";
    throw std::runtime_error("Current device configuration not supported, check device logs");
  }
...
bool destroyDevice(std::shared_ptr<boost::synchronized_value<ViamDeviceT>>& dev,
  ...
  try {
    if (not dev) {
      VIAM_DEVICE_LOG(logger, error) << "[destroyDevice] trying to destroy an unexistent device";
      return false;
    }
    {  // Begin scope for device lock
      ...
      if (device->started) {
        VIAM_DEVICE_LOG(logger, info) << "[destroyDevice] stopping pipe " << device->serial_number;
        device->pipe->stop();
    ...
    VIAM_DEVICE_LOG(logger, error)
        << "[destroyDevice] Exception caught while destroying device: " << e.what();
    dev = nullptr;
    return false;
  } catch (...) {
    VIAM_DEVICE_LOG(logger, error)
...
std::shared_ptr<boost::synchronized_value<ViamDeviceT>> createDevice(
  ...
  if (not camera_model) {
    VIAM_DEVICE_LOG(logger, error)
        << "[createDevice] Failed to register camera serial number: " << serial_number
        << " since no camera model found";
    return nullptr;
  }
  VIAM_DEVICE_LOG(logger, info) << "[createDevice] Found camera model: " << *camera_model;
  if (supported_camera_models.count(*camera_model) == 0) {
    VIAM_DEVICE_LOG(logger, error)
        << "[createDevice] Failed to register camera serial number: " << serial_number
        << " since camera model is not D435 or D435i, camera model: " << *camera_model;
    return nullptr;
  } else {
    VIAM_DEVICE_LOG(logger, info) << "[createDevice] Camera model is supported: " << *camera_model;
  }

  std::shared_ptr<ConfigT> config =
      createConfig<DeviceT, ConfigT, ColorSensorT, DepthSensorT, VideoStreamProfileT, ViamConfigT>(
          dev, viamConfig, logger);
  if (config == nullptr) {
    VIAM_DEVICE_LOG(logger, error)
        << "[createDevice] failed to create config for device: " << serial_number;
    return nullptr;
  }

  VIAM_DEVICE_LOG(logger, info) << "[createDevice] Config created for: " << serial_number;
...
void startDevice(std::string const& serialNumber,
                 ...
                 viam::sdk::LogSource& logger) {
  VIAM_DEVICE_LOG(logger, info) << "[startDevice] starting device " << serialNumber;
  if (not dev) {
    std::ostringstream buffer;
    buffer << "[startDevice] unable to start device " << serialNumber << " since "
  ...
  {  // Begin scope for dev_ptr lock
    ...
    if (dev_ptr->started) {
      std::ostringstream buffer;
      buffer << "[startDevice] unable to start already started device " << serialNumber;
      throw std::invalid_argument(buffer.str());
    }

    dev_ptr->config->enable_device(serialNumber);
    dev_ptr->pipe->start(*dev_ptr->config,
                         [maxFrameAgeMs, &frameSetStorage, viamConfig](auto const& frame) {
                           frameCallback(frame, maxFrameAgeMs, frameSetStorage, viamConfig);
                         });

    dev_ptr->started = true;
  }  // End scope for dev_ptr lock
  VIAM_DEVICE_LOG(logger, info) << "[startDevice]  device started " << serialNumber;
}
...
void reconfigureDevice(std::shared_ptr<boost::synchronized_value<ViamDeviceT>> dev,
  ...
  {  // Begin scope for device_guard lock
    ...
    auto new_config =
        createConfig<DeviceT, ConfigT, ColorSensorT, DepthSensorT, VideoStreamProfileT,
                     ViamConfigT>(device_guard->device, viamConfig, logger);
    if (new_config == nullptr) {
      VIAM_DEVICE_LOG(logger, error) << "[reconfigureDevice] failed to create new config";
      throw std::runtime_error("failed to create new config");
    }

    if (device_guard->started) {
      VIAM_DEVICE_LOG(logger, error) << "[reconfigureDevice] cannot reconfigure a started device";
...
bool stopDevice(std::shared_ptr<boost::synchronized_value<ViamDeviceT>>& dev,
  ...
  try {
    if (not dev) {
      VIAM_DEVICE_LOG(logger, error) << "[stopDevice] trying to stop a device that does not exist";
      return false;
    }
    {  // Begin scope for dev_ptr lock
      auto dev_ptr = dev->synchronize();
      if (not dev_ptr->started) {
        VIAM_DEVICE_LOG(logger, error) << "[stopDevice] unable to stop device that is not "
                                          "currently running "
    ...
    VIAM_DEVICE_LOG(logger, error) << "[stopDevice] Unknown exception caught while stopping device";
  }
  return false;
}
}  // namespace device
