...
std::vector<uint8_t> createValidZipWithBin(const std::string& filename,
                                           const std::vector<uint8_t>& content) {
  // Create a temporary zip file
  std::filesystem::path tempDir = std::filesystem::temp_directory_path();
  std::filesystem::path zipPath = tempDir / ("test_" + std::to_string(rand()) + ".zip");
  ...
  // Add file to zip
  zip_source_t* file_src = zip_source_buffer(zip, content.data(), content.size(), 0);
  if (!file_src) {
    zip_close(zip);
...
TEST_F(ZipUtilsTest, ExtractBinFromZip_ValidZipWithBinFile_ExtractsCorrectly) {
  ...
  std::vector<uint8_t> zipData = createValidZipWithBin("firmware.bin", binContent);
...
TEST_F(ZipUtilsTest, ExtractBinFromZip_BinInSubdirectory_ExtractsCorrectly) {
  ...
  std::vector<uint8_t> zipData = createValidZipWithBin("subdir/firmware.bin", binContent);
...
TEST_F(ZipUtilsTest, ExtractBinFromZip_InvalidZipData_ThrowsException) {
  ...
  EXPECT_THROW({
    extractBinFromZip(invalidData, logger_);
  }, std::runtime_error);
}
...
TEST_F(ZipUtilsTest, ExtractBinFromZip_EmptyZipData_ThrowsException) {
  ...
  EXPECT_THROW({
    extractBinFromZip(emptyData, logger_);
  }, std::runtime_error);
}
...
TEST_F(ZipUtilsTest, ExtractBinFromZip_ZipWithoutBinFile_ThrowsException) {
  ...
  std::vector<uint8_t> zipData = createValidZipWithBin("readme.txt", txtContent);

  EXPECT_THROW({
    extractBinFromZip(zipData, logger_);
  }, std::runtime_error);
}

TEST_F(ZipUtilsTest, ExtractBinFromZip_BinFileWithSpecialCharacters_ExtractsCorrectly) {
  std::vector<uint8_t> binContent = {0x00, 0xFF, 0x01, 0xFE, 0x7F, 0x80};
  std::vector<uint8_t> zipData = createValidZipWithBin("special-name_123.bin", binContent);
...
TEST_F(ZipUtilsTest, ExtractBinFromZip_UppercaseBinExtension_ExtractsCorrectly) {
  std::vector<uint8_t> binContent = {0xDE, 0xAD, 0xBE, 0xEF};
  // Note: The current implementation checks for lowercase .bin only
  // This test documents current behavior - if you want case-insensitive matching, update the implementation
  std::vector<uint8_t> zipData = createValidZipWithBin("FIRMWARE.BIN", binContent);
...
TEST_F(ZipUtilsTest, Integration_NonZipData_IsZipReturnsFalseAndExtractThrows) {
  ...
  EXPECT_THROW({
    extractBinFromZip(nonZipData, logger_);
  }, std::runtime_error);
}
...
TEST_F(ZipUtilsTest, Integration_RealWorldFirmwareFile_ExtractsCorrectly) {
  ...
  std::vector<uint8_t> zipData = createValidZipWithBin("D4XX_FW_Image-5.17.0.9.bin", firmwareData);
