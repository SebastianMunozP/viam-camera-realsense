...
TEST_F(EncodingTest, EncodeDepthRAWToResponse_ValidData) {
  auto result = encodeDepthRAWToResponse(test_depth_data_.data(), test_width_, test_height_);

  std::vector<uint8_t> expected_bytes{
      0x44, 0x45, 0x50, 0x54, 0x48, 0x4d, 0x41, 0x50, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
      0x00, 0x03, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x03, 0x03, 0xe8, 0x07, 0xd0,
      0x0b, 0xb8, 0x05, 0xdc, 0x09, 0xc4, 0x0d, 0xac, 0x07, 0xd0, 0x0b, 0xb8, 0x0f, 0xa0};
...
TEST_F(EncodingTest, EncodeJPEGToResponse_ValidData) {
  auto result = encodeJPEGToResponse(test_rgb_data_.data(), test_width_, test_height_);
...
TEST_F(EncodingTest, EncodeDepthRAWToResponse_ZeroDimensions) {
  EXPECT_THROW(
      { encodeDepthRAWToResponse(test_depth_data_.data(), 0, test_height_); }, std::runtime_error);

  EXPECT_THROW(
      { encodeDepthRAWToResponse(test_depth_data_.data(), test_width_, 0); }, std::runtime_error);
}
...
TEST_F(EncodingTest, EncodeJPEGToResponse_ZeroDimensions) {
  EXPECT_THROW(
      { encodeJPEGToResponse(test_rgb_data_.data(), 0, test_height_); }, std::runtime_error);

  EXPECT_THROW(
      { encodeJPEGToResponse(test_rgb_data_.data(), test_width_, 0); }, std::runtime_error);
}
...
TEST_F(EncodingTest, EncodeJPEGToResponse_ConsistentResults) {
  auto result1 = encodeJPEGToResponse(test_rgb_data_.data(), test_width_, test_height_);

  auto result2 = encodeJPEGToResponse(test_rgb_data_.data(), test_width_, test_height_);
...
TEST_F(EncodingTest, EncodeDepthRAWToResponse_ConsistentResults) {
  auto result1 = encodeDepthRAWToResponse(test_depth_data_.data(), test_width_, test_height_);

  auto result2 = encodeDepthRAWToResponse(test_depth_data_.data(), test_width_, test_height_);
