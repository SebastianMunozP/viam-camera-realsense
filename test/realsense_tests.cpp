#include <gtest/gtest.h>
#include <gmock/gmock.h>

#include "realsense.hpp"
#include "device.hpp"
#include <viam/sdk/config/resource.hpp>
#include <viam/sdk/components/camera.hpp>
#include <viam/sdk/common/instance.hpp>

#include <librealsense2/rs.hpp>

using namespace realsense;
using namespace viam::sdk;


class MockRsDevice : public rs2::device {
public:
    MockRsDevice(const std::string& serial) : serial_(serial) {}
    
    const char* get_info(rs2_camera_info info) const {
        if (info == RS2_CAMERA_INFO_SERIAL_NUMBER) {
            return serial_.c_str();
        }
        return "mock_info";
    }
    
    std::string get_serial() const { return serial_; }
    
private:
    std::string serial_;
};

class MockDeviceFunctions {
public:
    MOCK_METHOD(bool, stopDevice, 
                (boost::synchronized_value<std::shared_ptr<device::ViamRSDevice>>&), ());
    MOCK_METHOD(bool, destroyDevice, 
                (boost::synchronized_value<std::shared_ptr<device::ViamRSDevice>>&), ());
    MOCK_METHOD(void, printDeviceInfo, (const rs2::device&), ());
    MOCK_METHOD(std::shared_ptr<device::ViamRSDevice>, createDevice,
                (const std::string&, std::shared_ptr<rs2::device>, 
                 const std::unordered_set<std::string>&), ());
    MOCK_METHOD(void, startDevice,
                (const std::string&,
                 boost::synchronized_value<std::shared_ptr<device::ViamRSDevice>>&,
                 boost::synchronized_value<std::shared_ptr<rs2::frameset>>&,
                 std::uint64_t), ());
    MOCK_METHOD(void, deviceChangedCallback,
                (rs2::event_information&, const std::unordered_set<std::string>&,
                 boost::synchronized_value<std::shared_ptr<device::ViamRSDevice>>&,
                 const std::string&,
                 boost::synchronized_value<std::shared_ptr<rs2::frameset>>&,
                 std::uint64_t), ());
};

DeviceFunctions createMockDeviceFunctionsWithOrder(std::shared_ptr<MockDeviceFunctions> mock) {
    return DeviceFunctions{
        .stopDevice = [mock](boost::synchronized_value<std::shared_ptr<device::ViamRSDevice>>& device) -> bool {
            return mock->stopDevice(device);
        },
        .destroyDevice = [mock](boost::synchronized_value<std::shared_ptr<device::ViamRSDevice>>& device) -> bool {
            return mock->destroyDevice(device);
        },
        .printDeviceInfo = [mock](const auto& dev) {
            mock->printDeviceInfo(dev);
        },
        .createDevice = [mock](const std::string& serial, std::shared_ptr<rs2::device> dev_ptr,
                               const std::unordered_set<std::string>& supported_models) -> std::shared_ptr<device::ViamRSDevice> {
            return mock->createDevice(serial, dev_ptr, supported_models);
        },
        .startDevice = [mock](const std::string& serial,
                              boost::synchronized_value<std::shared_ptr<device::ViamRSDevice>>& device,
                              boost::synchronized_value<std::shared_ptr<rs2::frameset>>& latest_frameset,
                              std::uint64_t maxFrameAgeMs) {
            mock->startDevice(serial, device, latest_frameset, maxFrameAgeMs);
        },
        .deviceChangedCallback = [mock](rs2::event_information& info,
                                        const std::unordered_set<std::string>& supported_camera_models,
                                        boost::synchronized_value<std::shared_ptr<device::ViamRSDevice>>& device,
                                        const std::string& required_serial_number,
                                        boost::synchronized_value<std::shared_ptr<rs2::frameset>>& frame_set_storage,
                                        std::uint64_t maxFrameAgeMs) {
            mock->deviceChangedCallback(info, supported_camera_models, device, required_serial_number, frame_set_storage, maxFrameAgeMs);
        }
    };
}

DeviceFunctions createFullyMockedDeviceFunctions() {
    return DeviceFunctions{
        .stopDevice = [](boost::synchronized_value<std::shared_ptr<device::ViamRSDevice>>& device) -> bool {
            std::cout << "Mock: stopDevice called" << std::endl;
            auto locked_device = device.synchronize();
            if (*locked_device) {
                (*locked_device)->started = false;
            }
            return true;
        },
        .destroyDevice = [](boost::synchronized_value<std::shared_ptr<device::ViamRSDevice>>& device) -> bool {
            std::cout << "Mock: destroyDevice called" << std::endl;
            device = nullptr;
            return true;
        },
        .printDeviceInfo = [](const auto& dev) {
            std::cout << "Mock: printDeviceInfo called" << std::endl;
            // If it's our MockRsDevice, we could print its serial
            if constexpr (std::is_same_v<std::decay_t<decltype(dev)>, MockRsDevice>) {
                std::cout << "Mock device serial: " << dev.get_serial() << std::endl;
            }
        },
        .createDevice = [](const std::string& serial, std::shared_ptr<rs2::device> dev_ptr,
                           const std::unordered_set<std::string>& supported_models) -> std::shared_ptr<device::ViamRSDevice> {
            std::cout << "Mock: createDevice called for " << serial << std::endl;
            
            auto mock_device = std::make_shared<device::ViamRSDevice>();
            mock_device->serial_number = serial;
            mock_device->device = nullptr;  // Don't store the real device pointer
            mock_device->started = false;
            
            // IMPORTANT: Don't initialize real rs2 objects
            // mock_device->pipe = nullptr;
            // mock_device->align = nullptr;
            // mock_device->point_cloud_filter = nullptr;
            
            return mock_device;
        },
        .startDevice = [](const std::string& serial,
                          boost::synchronized_value<std::shared_ptr<device::ViamRSDevice>>& device,
                          boost::synchronized_value<std::shared_ptr<rs2::frameset>>& latest_frameset,
                          std::uint64_t maxFrameAgeMs) {
            std::cout << "Mock: startDevice called for " << serial << std::endl;
            auto locked_device = device.synchronize();
            if (*locked_device) {
                (*locked_device)->started = true;
            }
        },
        .deviceChangedCallback = [](rs2::event_information& info,
                                    const std::unordered_set<std::string>& supported_camera_models,
                                    boost::synchronized_value<std::shared_ptr<device::ViamRSDevice>>& device,
                                    const std::string& required_serial_number,
                                    boost::synchronized_value<std::shared_ptr<rs2::frameset>>& frame_set_storage,
                                    std::uint64_t maxFrameAgeMs) {
            std::cout << "Mock: deviceChangedCallback called" << std::endl;
        }
    };
}


class SimpleMockContext {
public:
    std::vector<MockRsDevice> mock_devices_;
    std::function<void(rs2::event_information&)> callback_;
    
    auto query_devices() const {
        return mock_devices_;
    }
    
    void set_devices_changed_callback(std::function<void(rs2::event_information&)> cb) {
        callback_ = cb;
    }
    
    void add_device(const std::string& serial) {
        auto it = std::find_if(mock_devices_.begin(), mock_devices_.end(),
                              [&serial](const MockRsDevice& dev) {
                                  return dev.get_serial() == serial;
                              });
        if (it == mock_devices_.end()) {
            mock_devices_.emplace_back(serial);
        }
    }
    
    void remove_device(const std::string& serial) {
        mock_devices_.erase(
            std::remove_if(mock_devices_.begin(), mock_devices_.end(),
                          [&serial](const MockRsDevice& dev) {
                              return dev.get_serial() == serial;
                          }),
            mock_devices_.end()
        );
    }
    
    void replace_device(const std::string& old_serial, const std::string& new_serial) {
        remove_device(old_serial);
        add_device(new_serial);
    }
    
    void clear_devices() {
        mock_devices_.clear();
    }
};

class RealsenseTestEnvironment : public ::testing::Environment {
public:
    void SetUp() override {
        instance_ = std::make_unique<Instance>();
    }
    
    void TearDown() override {
        instance_.reset();
    }
    
private:
    std::unique_ptr<Instance> instance_;
};

class RealsenseTest : public ::testing::Test {
protected:
    void SetUp() override {
        test_serial_ = "test_device_123456";
        test_name_ = "test_realsense_camera";
        
        auto attributes = ProtoStruct{};
        attributes["serial_number"] = test_serial_;
        
        test_config_ = std::make_unique<ResourceConfig>(
            "rdk:component:camera",     // api
            "",                         // namespace  
            test_name_,                 // name
            attributes,                 // attributes
            "",                         // implicitDependsOn
            Model("viam", "camera", "realsense"),  // model
            LinkConfig{},               // associatedResourceConfigs
            log_level::info            // logLevel
        );

        test_deps_ = Dependencies{};
        
        mock_context_ = std::make_shared<SimpleMockContext>();
        mock_context_->add_device(test_serial_);
        
        mock_realsense_context_ = std::make_shared<RealsenseContext<SimpleMockContext>>(mock_context_);
    }
    
    void TearDown() override {
        // Cleanup happens automatically
    }
    
    std::string test_serial_;
    std::string test_name_;
    std::unique_ptr<ResourceConfig> test_config_;
    Dependencies test_deps_;
    std::shared_ptr<SimpleMockContext> mock_context_;
    std::shared_ptr<RealsenseContext<SimpleMockContext>> mock_realsense_context_;
};

TEST_F(RealsenseTest, ValidateWithValidConfig) {
    auto attributes = ProtoStruct{};
    attributes["serial_number"] = test_serial_;
    ResourceConfig valid_config(
        "rdk:component:camera", "", test_name_, attributes, "",
        Model("viam", "camera", "realsense"), LinkConfig{}, log_level::info
    );
    
    EXPECT_NO_THROW({
        auto result = Realsense<SimpleMockContext>::validate(valid_config);
        EXPECT_TRUE(result.empty());  // No validation errors
    });
}

TEST_F(RealsenseTest, ValidateWithInvalidConfig_NoSerialNumber) {
    auto empty_attributes = ProtoStruct{};
    ResourceConfig invalid_config(
        "rdk:component:camera", "", test_name_, empty_attributes, "",
        Model("viam", "camera", "realsense"), LinkConfig{}, log_level::info
    );
    
    EXPECT_THROW({
        Realsense<SimpleMockContext>::validate(invalid_config);
    }, std::invalid_argument);
}

TEST_F(RealsenseTest, ValidateWithInvalidConfig_EmptySerialNumber) {
    auto empty_serial_attributes = ProtoStruct{};
    empty_serial_attributes["serial_number"] = "";
    ResourceConfig invalid_config(
        "rdk:component:camera", "", test_name_, empty_serial_attributes, "",
        Model("viam", "camera", "realsense"), LinkConfig{}, log_level::info
    );
    
    EXPECT_THROW({
        Realsense<SimpleMockContext>::validate(invalid_config);
    }, std::invalid_argument);
}

TEST_F(RealsenseTest, DoCommandReturnsEmptyStruct) {
    Realsense<SimpleMockContext> camera(
        test_deps_, 
        *test_config_, 
        mock_realsense_context_, 
        createFullyMockedDeviceFunctions()
    );
    
    ProtoStruct command{};
    auto result = camera.do_command(command);
    
    // do_command should return an empty ProtoStruct since it's not implemented
    EXPECT_TRUE(result.empty());
}

TEST_F(RealsenseTest, GetGeometriesReturnsExpectedGeometry) {
    Realsense<SimpleMockContext> camera(
        test_deps_, 
        *test_config_, 
        mock_realsense_context_, 
        createFullyMockedDeviceFunctions()
    );
    
    ProtoStruct extra{};
    auto geometries = camera.get_geometries(extra);
    
    EXPECT_EQ(geometries.size(), 1);
    EXPECT_EQ(geometries, std::vector<viam::sdk::GeometryConfig>{
        viam::sdk::GeometryConfig(viam::sdk::pose{-17.5, 0, -12.5},
                                  viam::sdk::box({90, 25, 25}), "box")
    });

}

TEST(RealsenseStaticTest, ModelExists) {
    auto& model = Realsense<rs2::context>::model;
    
    // Test that we can access the model without errors
    EXPECT_NO_THROW({
        auto model_copy = model;  // Test copy constructor
    });
    EXPECT_EQ(model.to_string(), "viam:camera:realsense");
}

TEST(RsResourceConfigTest, ConstructorSetsCorrectValues) {
    std::string serial = "test_serial_123";
    std::string name = "test_camera";
    
    RsResourceConfig config(serial, name);
    
    EXPECT_EQ(config.serial_number, serial);
    EXPECT_EQ(config.resource_name, name);
}

TEST(RealsenseTemplateTest, CanInstantiateWithRealContext) {
    // This is just a compilation test - we don't actually create the object
    using RealRealsense = Realsense<rs2::context>;
    
    // If this compiles, the template instantiation works
    static_assert(std::is_same_v<decltype(RealRealsense::model), viam::sdk::Model>);
}

TEST_F(RealsenseTest, ReconfigureWithSameSerialNumber_StrictOrdering) {
    using ::testing::InSequence;
    using ::testing::Return;
    using ::testing::_;
    
    auto mock_device_funcs = std::make_shared<MockDeviceFunctions>();
    
    {
        InSequence seq;
        
        // Constructor sequence
        EXPECT_CALL(*mock_device_funcs, printDeviceInfo(_)).Times(1);
        EXPECT_CALL(*mock_device_funcs, createDevice("test_device_123456", _, _)).Times(1);
        EXPECT_CALL(*mock_device_funcs, startDevice("test_device_123456", _, _, _)).Times(1);
        
        // Reconfigure sequence
        EXPECT_CALL(*mock_device_funcs, stopDevice(_)).Times(1).WillOnce(Return(true));
        EXPECT_CALL(*mock_device_funcs, destroyDevice(_)).Times(1).WillOnce(Return(true));
        EXPECT_CALL(*mock_device_funcs, printDeviceInfo(_)).Times(1);  // Only new device
        EXPECT_CALL(*mock_device_funcs, createDevice("test_device_123456", _, _)).Times(1);
        EXPECT_CALL(*mock_device_funcs, startDevice("test_device_123456", _, _, _)).Times(1);
        
        EXPECT_CALL(*mock_device_funcs, stopDevice(_)).Times(1).WillOnce(Return(true));
        EXPECT_CALL(*mock_device_funcs, destroyDevice(_)).Times(1).WillOnce(Return(true));
    }
    
    auto isolated_context = std::make_shared<SimpleMockContext>();
    isolated_context->add_device("test_device_123456");
    auto isolated_realsense_context = std::make_shared<RealsenseContext<SimpleMockContext>>(isolated_context);
    
    Realsense<SimpleMockContext> camera(
        test_deps_, 
        *test_config_, 
        isolated_realsense_context, 
        createMockDeviceFunctionsWithOrder(mock_device_funcs)
    );
    
    EXPECT_NO_THROW({
        camera.reconfigure(test_deps_, *test_config_);
    });
}

TEST_F(RealsenseTest, ReconfigureWithNewSerialNumber_StrictOrdering) {
    using ::testing::InSequence;
    using ::testing::Return;
    using ::testing::_;
    
    auto mock_device_funcs = std::make_shared<MockDeviceFunctions>();
    
    {
        InSequence seq;
        
        // Constructor sequence
        EXPECT_CALL(*mock_device_funcs, printDeviceInfo(_)).Times(1);
        EXPECT_CALL(*mock_device_funcs, createDevice("test_device_123456", _, _)).Times(1);
        EXPECT_CALL(*mock_device_funcs, startDevice("test_device_123456", _, _, _)).Times(1);
        
        // Reconfigure sequence
        EXPECT_CALL(*mock_device_funcs, stopDevice(_)).Times(1).WillOnce(Return(true));
        EXPECT_CALL(*mock_device_funcs, destroyDevice(_)).Times(1).WillOnce(Return(true));
        EXPECT_CALL(*mock_device_funcs, printDeviceInfo(_)).Times(1);  // Only new device
        EXPECT_CALL(*mock_device_funcs, createDevice("new_device_789", _, _)).Times(1);
        EXPECT_CALL(*mock_device_funcs, startDevice("new_device_789", _, _, _)).Times(1);
        
        EXPECT_CALL(*mock_device_funcs, stopDevice(_)).Times(1).WillOnce(Return(true));
        EXPECT_CALL(*mock_device_funcs, destroyDevice(_)).Times(1).WillOnce(Return(true));
    }
    
    auto isolated_context = std::make_shared<SimpleMockContext>();
    isolated_context->add_device("test_device_123456");
    auto isolated_realsense_context = std::make_shared<RealsenseContext<SimpleMockContext>>(isolated_context);
    
    Realsense<SimpleMockContext> camera(
        test_deps_, 
        *test_config_, 
        isolated_realsense_context, 
        createMockDeviceFunctionsWithOrder(mock_device_funcs)
    );
    
    isolated_context->replace_device("test_device_123456", "new_device_789");
    
    auto new_attributes = ProtoStruct{};
    new_attributes["serial_number"] = "new_device_789";
    
    ResourceConfig new_config(
        "rdk:component:camera", "", "new_camera", new_attributes, "",
        Model("viam", "camera", "realsense"), LinkConfig{}, log_level::info
    );
    
    EXPECT_NO_THROW({
        camera.reconfigure(test_deps_, new_config);
    });
}

int main(int argc, char **argv) {
    ::testing::InitGoogleTest(&argc, argv);
    ::testing::AddGlobalTestEnvironment(new RealsenseTestEnvironment);
    return RUN_ALL_TESTS();
}